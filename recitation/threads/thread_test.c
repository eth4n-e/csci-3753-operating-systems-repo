...
static void read_label_bounded(char *dst, size_t cap) {
  ...
  if (truncated) {
    fprintf(stderr,
            "[warn] input longer than %zu chars; truncated and flushing.\n",
            cap - 1);
    int c;
    while ((c = getchar()) != '\n' && c != EOF) {
    }
...
static void *fn_bounds(void *arg) {
  ...
  if (need2 >= (int)sizeof(local))
    fprintf(stderr, "[warn] local truncated for \"%s\"\n", a->name);
...
int main(void) {
  ...
  for (int i = 0; i < THREADS; i++)
    pthread_create(&t[i], NULL, inc_no_lock, NULL);
  for (int i = 0; i < THREADS; i++)
  ...
  counter = 0;
  for (int i = 0; i < THREADS; i++)
    pthread_create(&t[i], NULL, inc_with_lock, NULL);
  for (int i = 0; i < THREADS; i++)
    pthread_join(t[i], NULL);
  printf("Expected: %d, got: %ld ✅\n", THREADS * ITERATIONS, counter);
  ...
  char *p1 = upper_not_reentrant("hello");
  printf("First call -> %s\n", p1);
  char *p2 = upper_not_reentrant("world");
  printf("Second call -> %s (overwrote first)\n", p2);
  ...
  char label[16];
  read_label_bounded(label, sizeof(label));
  char tag[20];
  int need = snprintf(tag, sizeof(tag), "TAG:%s", label);
  if (need >= (int)sizeof(tag))
    fprintf(stderr, "[warn] tag truncated\n");
  printf("Safe tag = \"%s\"\n", tag);
...
Part 3 — Why does second call overwrite first?
    ...
    • The pointer from the first call points to the same memory reused by the s
cond. • So the contents are replaced.
...
Part 5 — How to detect truncation & avoid shared state?
    ...
    • Avoid shared buffers by giving each thread its own local or caller-owned b
ffer.
...
Part 6 — What do empty/full count? Why a mutex too?
    ...
    • Mutex still needed to protect the actual array indices and memory from r
ces.
*/
